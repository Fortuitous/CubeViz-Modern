...
/*	start = Date.now();*/
    ...
    var orgtype = "Square"
    var cubeLevel = 2
    console.log("1: cube level = " + cubeLevel)
...
/*    $('#browseStart').on('vclick', function(){
    ...
    $('input[name="radio-choice-h-2"]').change(function(){
        mlength = $(this).val();
        console.log(typeof mlength)
        showCContext(orgtype, sortdt, mlength, cubeLevel)
        if (storageAvailable('localStorage')) { localStorage.mlength = "" + mlength }
    });
    $('input[name="radio-choice-h-1"]').change(function(){
        sortdt = $(this).val();
        showCContext(orgtype, sortdt, mlength, cubeLevel)
        if (storageAvailable('localStorage')) { localStorage.sortdt = sortdt }
    });

    $('input[name="radio-choice-h-3"]').change(function(){
        orgtype = $(this).val();
        showCContext(orgtype, sortdt, mlength, cubeLevel)
        if (storageAvailable('localStorage')) { localStorage.orgtype = orgtype }
    });

    $('input[name="radio-choice-h-4"]').change(function(){
        cubeLevel = $(this).val();
...
/*        if (current = "Cube Context") {$( "[data-role='header'] h1" ).html('<span class="headleft">Money</span><span class="headright">Scores</span>')}*/
/*        else {
            var headerText = getheaderText();
            $( "[data-role='header'] h1" ).text(headerText)
        }        */
...
/*            var deckOrder = [30, 0, 1, 2, 3, 4, 5, 6, 7, 8, 18, 9, 10, 11, 12, 13, 14, 15, 16, 17, 27, 19, 20, 21, 22, 23, 24, 25, 26, 28, 29, 40, 31, 32, 33, 34, 35, 36, 37, 38, 39, 41, 42, 43] */
            var deckOrder = [30, 41, 42, 43, 44, 45, 46]             
...
/*                    $('label[for="Deck' + l +'"]').css('display', 'none');*/
    ...
    function markSettings(){  //Startup
		setTimeout(function () {
            ...
            $("input:radio[name='radio-choice-h-3']").each(function() { $(this).prop('checked', false); });
            $("input:radio[name='radio-choice-h-3'][value ='"+orgtype+"']").prop('checked', true).checkboxradio("refresh");            
...
/*    function changemlength(mlength){
        showCContext(sortdt)
        var table = document.getElementById("cubecontext");
        for (var i = 0, row; row = table.rows[i]; i++) {
            if (Number(row.className) > mlength){
...
/*        var copyBobBtn = document.querySelector('.js-copy-bob-btn'),
    ...
    function orgtypeHide(orgtype){
        console.log("Hiding orgtype")
...
/*        answertext[answertext.length] = "<tr><td>&nbsp;</td></tr>"
    ...
    function showCContext(orgtype, sortdt, mlength, cubeLevel){

        orgtypeHide(orgtype)
        $('#cubecontext').empty();
        if (orgtype == "Table"){

            ccrow = "<tr><th></th><th></th><th></th><th colspan='2'>Double to 2</th><th colspan='2'></th><th colspan='2'>Double to 4</th><th colspan='2'></th><th colspan='2'>Double to 8</th><th colspan='2'></th><th colspan='2'>Double to 16</th><th colspan='2'></th></tr><tr style='border-bottom: 1px solid #ddd'><th>Score</th><th></th><th></th><th>Double?</th><th>Take?</th><th></th><th></th><th>Redouble?</th><th>Take?</th><th></th><th></th><th>Redouble?</th><th>Take?</th><th></th><th></th><th>Redouble?</th><th>Take?</th></tr>"
            $('#cubecontext').append(ccrow);

            truelinecount = -1

            for (var myneed=13; myneed>1; myneed--){
                for (var youneed=13; youneed>1; youneed--){
                    truelinecount += 1
                    ...
                    if (sortdt == "SortD"){
                        ...
                        linecount = (13-myneed)+((13-youneed)*12)
                        myneedshow = youneed
                        youneedshow = myneed
                    }
                    ...
                    for (var i=0; i<4; i++){
                        ...
                        else if (DubErr[i].value < 0){DubErr[i].color = 'red'}
    /*                    else if (DubErr[i].value < 0){DubErr[i].color = '#800000'}
                        else if (DubErr[i].value == 0){DubErr[i].color = 'white'}
                        else if (DubErr[i].value < .05){DubErr[i].color = '#196719'}        */                
                        else if (DubErr[i].value >= 0){DubErr[i].color = 'limegreen'}
                        ...
                        if (TakeErr[i].value < -100){
                            TakeErr[i].value = '---'
                            TakeErr[i].color = 'blue'
                        }
                        else if (TakeErr[i].value < 0){TakeErr[i].color = 'red'}
     /*                   else if (TakeErr[i].value < 0){TakeErr[i].color = '#800000'}
                        else if (TakeErr[i].value == 0){TakeErr[i].color = 'white'}
                        else if (TakeErr[i].value < .05){TakeErr[i].color = '#196719'}         */               
                        else if (TakeErr[i].value >= 0){TakeErr[i].color = 'limegreen'}
                    ...
                    ccrow = "<tr class='" + minneed + "'><th style='min-width:75px'>-" + myneedshow + ", -" + youneedshow + "</th><td></td><td></td>"
                    for (var i=0; i<4; i++){
                        ccrow += "<td style='color:" + DubErr[i].color +"'>" + DubErr[i].value + "</td><td style='color:" + TakeErr[i].color +"'>" + TakeErr[i].value + "</td>"
                        if (i < 3) {
                            ccrow += "<td></td><td></td>"
                        }
                        else {
                ...
                ccrow = "<tr class='" + minneed + "'><td colspan='19'><hr></td></tr>"
                $('#cubecontext').append(ccrow);
            }
            ...
            ccrow = "<tr><th></th><th></th><th colspan='4'>Taker Needs</th></tr>"
            $('#cubecontext').append(ccrow);

            ccrow = "<tr style='border-bottom: 1px solid #ddd'><th></th>"
            for (var ml = 2; ml <= mlength; ml++){
                console.log("mlength  = " + mlength)
                console.log("ml = " + ml)
                ccrow += "<th>" + ml + "</th>"
            }
...
/*                    else if (DubErr[i].value < 0){DubErr[i].color = 'red'}*/
                    else if (DubErr[i].value < 0){DubErr[i].color = '#2a2a2a'}                        
/*                    else if (DubErr[i].value < 0){DubErr[i].color = '#800000'}
                    else if (DubErr[i].value == 0){DubErr[i].color = 'white'}
                    else if (DubErr[i].value < .05){DubErr[i].color = '#196719'}        */                
/*                    else if (DubErr[i].value >= 0){DubErr[i].color = 'limegreen'}*/
                    ...
                    DubErr[i].value = DubErr[i].value.toString().replace("0.", "&nbsp.")
                    DubErr[i].value = DubErr[i].value.toString().replace("-0.", "-&nbsp.")                    
                    DubErr[i].value = DubErr[i].value.toString().replace("-", "")                    

                    if (TakeErr[i].value < -100){
                        TakeErr[i].value = '----'
                        TakeErr[i].color = 'blue'
                    }
                    else if (TakeErr[i].value < -.026){TakeErr[i].color = 'red'}
 /*                   else if (TakeErr[i].value < 0){TakeErr[i].color = '#800000'}
                    else if (TakeErr[i].value == 0){TakeErr[i].color = 'white'}
                    else if (TakeErr[i].value < .05){TakeErr[i].color = '#196719'}         */               
                    else if (TakeErr[i].value >= .027){TakeErr[i].color = 'limegreen'}
                    else {TakeErr[i].color = 'white'}

                    TakeErr[i].value = TakeErr[i].value.toString().replace("0.", "&nbsp.")
                    TakeErr[i].value = TakeErr[i].value.toString().replace("-0.", "-&nbsp.")                    
/*                    TakeErr[i].value = TakeErr[i].value.toString().replace("-", "")                    */
                   
/*                    ccrow += "<td style='color:" + DubErr[i].color +"'>" + DubErr[i].value + "</td><td style='color:" + TakeErr[i].color +"'>" + TakeErr[i].value + "</td>"*/
/*                    ccrow += "<td><span style='color:" + DubErr[i].color +"'>" + DubErr[i].value + " </span><span style='color:white'>/ </span><span style='color:" + TakeErr[i].color +"'>" + TakeErr[i].value + "</span></td>"*/
                    ccrow += "<td ><span style='white-space: nowrap; color:" + DubErr[i].color +"'>" + DubErr[i].value + "&nbsp&nbsp&nbsp&nbsp</span><br><span style='color:" + TakeErr[i].color +"; float:right'>" + TakeErr[i].value + "</span></td>"                    

                }
                ccrow +="</tr>"
                $('#cubecontext').append(ccrow); 
                truelinecount = truelinecount - (13-mlength)
...
/*        cubeData.moneyNDEr = ((Math.min(cubeData.moneyDT, 1)) - cubeData.moneyND  ).toFixed(3)
        cubeData.moneyNRDEr = ((Math.min(cubeData.moneyDT, 1)) - cubeData.moneyNRD ).toFixed(3)
...
/*        if (Math.abs(eqDiff) > .01) {acubeColor = "green"} //#009933"}
        ...
        if (Number(eqDiff) > 0) {acubeColor = "limegreen"} //#009933"}
        else {acubeColor = "red"} //#FF0000"}            
        return acubeColor
    }
