...
/*	start = Date.now();*/
    ...
    var posColorTable = ["#f7fbff","#e3eef9","#cfe1f2","#b5d4e9","#93c3df","#6daed5","#4b97c9","#2f7ebc","#1864aa","#0a4a90","#08306b"] // blues
    //var posColorTable = ["#f7fcf5","#e8f6e3","#d3eecd","#b7e2b1","#97d494","#73c378","#4daf62","#2f984f","#157f3b","#036429","#00441b"] // greens
    //var colorTable = ["#ffffff","#f2f2f2","#e2e2e2","#cecece","#b4b4b4","#979797","#7a7a7a","#5f5f5f","#404040","#1e1e1e","#000000"] // greys
    //var negColorTable = ["#fff5eb","#fee8d3","#fdd8b3","#fdc28c","#fda762","#fb8d3d","#f2701d","#e25609","#c44103","#9f3303","#7f2704"] // oranges
    //var negColorTable = ["#fcfbfd","#f1eff6","#e2e1ef","#cecee5","#b6b5d8","#9e9bc9","#8782bc","#7363ac","#61409b","#501f8c","#3f007d"] // purples
    var negColorTable = ["#fff5f0","#fee3d6","#fdc9b4","#fcaa8e","#fc8a6b","#f9694c","#ef4533","#d92723","#bb151a","#970b13","#67000d"] // reds
/*    animateFlag = false*/
...
/*    $('#browseStart').on('vclick', function(){
    ...
    $('input[name="radio-choice-h-2"]').change(function(){ //Match Length
        ...
        showCContext(orgtype, sortdt, mlength, cubeLevel, showCell)
    ...
    $('input[name="radio-choice-h-3"]').change(function(){  //Org Type Square or Table.  Now only Square.
        orgtype = $(this).val();
        showCContext(orgtype, sortdt, mlength, cubeLevel, showCell)
...
/*            var deckOrder = [30, 0, 1, 2, 3, 4, 5, 6, 7, 8, 18, 9, 10, 11, 12, 13, 14, 15, 16, 17, 27, 19, 20, 21, 22, 23, 24, 25, 26, 28, 29, 40, 31, 32, 33, 34, 35, 36, 37, 38, 39, 41, 42, 43] */
            var deckOrder = [30, 41, 42, 43, 44, 45, 46, 47]             
...
/*        if (fbSSFlag) bglog.swapSides(); // reset swapSides if necessary*/
 ...
 /*           setTimeout(function () {
                if (swapSidesFlag) bglog.swapSides();        
            }, 0);
*/            
            // reset swapSides if necessary
/*			if (chosencard.CardKind == 0){
...
/*            animateFlag = true*/
    ...
    function showCContext(orgtype, sortdt, mlength, cubeLevel, showCell){
...
/*            ccrow = "<tr style='border-bottom: 1px solid #ddd'><th>Taker Needs</th>"*/
...
/*                    else if (DubErr[i].value >= 0){DubErr[i].color = 'limegreen'}*/
					...
					if (TakeErr[i].value < 0){
						if (TakeErr[i].value < -100){
							TakeErr[i].value = '----'
							TakeErr[i].color = 'grey'
						} else {
							for (j=0; j<negColorTable.length; j++){
								if (TakeErr[i].value < takeErrNeg[j]) TakeErr[i].color = negColorTable[j]
							}
						}
					} else if (TakeErr[i].value > 0){
						for (j=0; j<posColorTable.length; j++){
							if (TakeErr[i].value > takeErrPos[j]) TakeErr[i].color = posColorTable[j]
						}
        ...
        //Show Legend
        lrow = "<tr><td colspan='12' style='text-align: center'>Take / Pass by up to:</td></tr>" 
        ...
        for (j=0; j<negColorTable.length; j++){
           ...
           value = value.toString().replace("-0.", "-.")
           textColor = "#555"
           if (lightOrDark(posColorTable[j]) == "dark") textColor = "#ccc"
           lrow += "<td style='background-color: " + posColorTable[j] + "; width: 30px; height: 30px; font-size: 12px; color: " + textColor + "; white-space: nowrap;'>" + value + "</td>"      
        }
        lrow += "</tr>"
        $('#legendtable').append(lrow);

        lrow = "<tr><td>P&nbsp</td>"
        for (j=0; j<negColorTable.length; j++){
           var value = takeErrNeg[j] 
           value = value.toString().replace("0.", ".")
           value = value.toString().replace("-0.", "-.")
           textColor = "#555"
           if (lightOrDark(negColorTable[j]) == "dark") textColor = "#ccc"           
           lrow += "<td style='background-color: " + negColorTable[j] + "; width: 30px; height: 30px; font-size: 12px; color: " + textColor + "; white-space: nowrap;'></td>"      
        }
...
/*        if (Number(eqDiff) > 0) {acubeColor = "limegreen"} //#009933"}
        ...
        return acubeColor
*/
        if (Number(eqDiff) > 0) {acubeColor = posColorTable[6]} //#009933"}
        else {acubeColor = negColorTable[6]} //#FF0000"}            
        return acubeColor
