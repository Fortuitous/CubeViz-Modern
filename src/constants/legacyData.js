export const DECK_SET_ORDER = [10, 12, 9, 13, 11, 14, 15];

export const DECK_ORDER = [
  30, 41, 42, 43, 47, 44, 45, 46, 50, 48, 49, 51, 52, 54, 55, 56, 57, 58, 59
];

/**
 * Hardcoded list of valid CardInDeck indices from legacy flashcheck.js.
 * This list filters out stale or unwanted "foreign" positions that exist in the raw data.
 */
export const CC_CARD_ORDER = [
  6842, 6843, 6844, 6845, 6846, 6847, 6848, 6849, 
  6850, 6851, 6852, 6853, 6855, 6856, 6857, 6859, 
  6860, 6861, 6862, 6863, 6864, 6865, 6866, 6867, 
  6868, 6869, 6870, 6871, 6872, 6873, 6874, 6875, 
  6876, 6877, 6878, 
  6903, 6904, 6905, 6906, 6907, 6908, 6909, 6910, 
  6911, 6912, 6913, 6914, 6915, 6916, 6917, 6918, 
  6919, 6920, 6921, 6922, 6923, 6924, 6925, 6926, 
  6927, 6928, 6929, 6930, 6931, 6932, 6933, 6934, 
  6935, 6936, 6937, 6938, 6939, 6940, 6941, 6942, 
  6943, 6944, 6945
];
