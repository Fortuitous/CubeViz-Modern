...
function onDeviceReady() {
    ...
    /*$('#clickme').on('click', function(){ loadcard(); });*/
    $('#hard').on('click', function () { ishard(); });
    $('#unsure').on('click', function () { isunsure(); });
    $('#easy').on('click', function () { iseasy(); });
    $('#reset').on('click', function () { loadCard(); });
    $('#cleartrain').on('click', function (e) {
        e.preventDefault();
        clearTraining();
    });
    $('#clearKeys').on('click', function () { clearTrainingKeys(); });
    $('#clearselectedtrain').on('click', function (e) {
        e.preventDefault();
        clearSelectedTraining();
    });
    $('#clearSelectedKeys').on('click', function () { clearSelectedTrainingKeys(); });
    $('#copyXGButton').on('click', function () {
        copyXG();
    ...
    $('#beginBtn').on('click', function () {
        currentbrowse = 0
        $("#prevBtn").text('Previous (Home)')
        areAnyDecksSelected();
    });
    $('#nextBtn').on('click', function () { browseResult(1); });
    $('#prevBtn').on('click', function () { browseResult(-1); });
    ...
    $('#swapButton').on('click', function () { fbswap(); return false; });
    /*    $('#swapButton').on('hover', function() { fbswap(); return false;});  */


    $('#questioncontainer').on('click', function () { toggleAnswerContainer(); });
    $('#clearLS').on('click', function () { clearLS(); }); // Dev only option
    $('#hintButton').on('click', function () { openHint(); });
    $('#restorePurchases').on('click', function () { restorePurchases(); });
    $('#clearDecks').on('click', function () { clearSelectedDecks(); });

    $('#settingsHeaderButton').on('click', function () {
        $("body").pagecontainer("change", "#settings", { transition: "pop" });
    });

    $('#otherHeaderButton').on('click', function () { otherHeaderFunction() });

    $('#homeHeaderButton').on('click', function () {
        $('#flashcards').hide(); $('#deckselect').show(); bglog.draw();
    });

    $('#infoHeaderButton').on('click', function () { infoHeaderFunction() });
    ...
    function init() {
        ...
        //        animateFlag = false;
        $('#flashcards').show(); $('#deckselect').hide(); bglog.makeBoard();
        changeTheme(fbTheme);
        changeUITheme(fbUITheme);
    ...
    function createDeckMenu() {
        ...
        for (var h = deckSetOrder.length; h > 0; h--) {
            i = h - 1
            var content = '<fieldset class="decksetclass" id="' + DeckSet[deckSetOrder[i]].DeckSetName + '"><legend>' + DeckSet[deckSetOrder[i]].DeckSetName + '</legend><div class="insidedecksetclass" id="Inside' + deckSetOrder[i] + '"></div></fieldset>'

            $(content).prependTo('.DSContent')
        ...
        for (var k = 0; k < DeckSet.length; k++) {
            if (fbDeckSetForSale[k] == false) {
                $('#buyButton' + k).hide(); $('#buying' + k).hide();
    ...
    function checkFavorites() {
        if (fave.length == 0) {
            $('#Deck30').prop('checked', false);
            $('#Deck30').prop('disabled', true);
    ...
    function markSettings() {  //Startup
        setTimeout(function () {
            ...
            $("input:radio[name='fbTheme'][value ='" + fbTheme + "']").prop('checked', true);
            $("input:radio[name='fbDirection']").each(function () { $(this).prop('checked', false); });
            $("input:radio[name='fbDirection'][value ='" + fbDirection + "']").prop('checked', true);

            $("input:radio[name='fbUITheme']").each(function () { $(this).prop('checked', false); });
            $("input:radio[name='fbUITheme'][value ='" + fbUITheme + "']").prop('checked', true);



            $("input:radio[name='radio-choice-h-2']").each(function () { $(this).prop('checked', false); });
            $("input:radio[name='radio-choice-h-2'][value ='" + mlength + "']").prop('checked', true);
            $("input:radio[name='radio-choice-h-1']").each(function () { $(this).prop('checked', false); });
            $("input:radio[name='radio-choice-h-1'][value ='" + sortdt + "']").prop('checked', true);
            $("input:radio[name='radio-choice-h-6']").each(function () { $(this).prop('checked', false); });
            $("input:radio[name='radio-choice-h-6'][value ='" + dataType + "']").prop('checked', true);
            $("input:radio[name='radio-choice-h-4']").each(function () { $(this).prop('checked', false); });
            $("input:radio[name='radio-choice-h-4'][value ='" + cubeLevel + "']").prop('checked', true);
            $("input:radio[name='radio-choice-h-5']").each(function () { $(this).prop('checked', false); });
            $("input:radio[name='radio-choice-h-5'][value ='" + showCell + "']").prop('checked', true);





            $('#fbShowDice').prop('checked', fbShowDice);
            $('#fbShowDeckName').prop('checked', fbShowDeckName);
            $('#fbShowPipcount').prop('checked', fbShowPipcount);
            $('#fbShowPointNumbers').prop('checked', fbShowPointNumbers);
            $('#fbShowTakiPoints').prop('checked', fbShowTakiPoints);
    ...
    function areAnyDecksSelected() {
        ...
        try {
            ...
            xhr.open("POST", "http://127.0.0.1:8888/", false);
            xhr.send("STACK: " + err.stack);
        }
    }
    ...
    function loadCard() {
        ...
        $('#deckselect').hide(); $('#flashcards').show(); bglog.draw();
    ...
    function showAnswer() {
        ...
        $('#favButton').on('click', function () { manageFav(); });
